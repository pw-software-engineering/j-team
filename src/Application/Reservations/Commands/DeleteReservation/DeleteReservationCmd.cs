    ...
    public class DeleteReservationCmdHandler : IRequestHandler<DeleteReservationCmd, int>
    {
        ...
        public async Task<int> Handle(DeleteReservationCmd request, CancellationToken cancellationToken)
        {
            ...
            if (DateTime.Compare(reservation.FromTime, DateTime.Now) < 0)
                throw new ValidationException(new ValidationFailure[]{new ValidationFailure(nameof(Reservation), "Reservation is currently underway or already completed")});
            ...
            var intersectedReservations = _context.Reservations.Where(r =>
                ...
                DateTime.Compare(r.ToTime, reservation.FromTime) > 0).ToList();
            /*List<CreateReservationCmd> reservationsCmds = new List<CreateReservationCmd>();
            foreach (var intersectedReservation in intersectedReservations)
            {
                _context.Reservations.Remove(intersectedReservation);
                reservationsCmds.Add(new CreateReservationCmd
                {
                    ClientId = intersectedReservation.ClientId,
                    From = intersectedReservation.FromTime,
                    To = intersectedReservation.ToTime,
                    NumberOfAdults = intersectedReservation.AdultsCount,
                    NumberOfChildren = intersectedReservation.ChildrenCount,
                    OfferId = intersectedReservation.OfferId
                });
            }*/
                    ...
                    .Where(room => (room.Offers.Where(o => o.OfferId == intersectedReservation.OfferId).Any()
                                    && !room.Reservations.Where(r =>
                                            (r.ToTime.CompareTo(intersectedReservation.FromTime) >= 0
                                             && r.FromTime.CompareTo(intersectedReservation.ToTime) <= 0)).Any())).ToList();
